...
LatData    = ncread(fullfile(claus_path,claus_name),'latitude');
LonData    = ncread(fullfile(claus_path, claus_name),'longitude');
latdata = LatData(180:360);
londata = LonData(1:1080);
...
for i=1:length(record_tracks)
    ...
    for j = 1:track.age
        ...
        MCS_record(i).gx(j) = track.list_shield_x(j);
        MCS_record(i).gy(j) = track.list_shield_y(j);
        MCS_record(i).wx(j) = track.list_core_x(j);
        MCS_record(i).wy(j) = track.list_core_y(j);
