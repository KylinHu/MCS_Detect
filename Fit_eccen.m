function eccentricity = Fit_eccen(x,y)
% function ellipse_t = fit_ellipse( x,y,axis_handle )

% https://cn.mathworks.com/matlabcentral/fileexchange/3215-fit-ellipse?s_tid=srchtitle
...
switch (1)
case (test>0),  status = '';
case (test==0), status = 'Parabola found'; eccentricity = NaN; %warning( 'fit_ellipse: Did not locate an ellipse' );
case (test<0),  status = 'Hyperbola found';eccentricity = NaN; %warning( 'fit_ellipse: Did not locate an ellipse' );
end
...
%     short_axis  = 2*min(a,b);
    ...
    eccentricity = short_axis/long_axis ;
